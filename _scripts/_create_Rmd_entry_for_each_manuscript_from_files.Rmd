...
tibble(
  ...
  filesize = map_dbl(aws_contents, "Size"),
...
```{r}
write_yaml_header <- function(line_of_meta_data, is_text_only_version = FALSE){
  text_only_line <- ""
...
hasText: {has_text}{text_only_line}
categories:
  - sanskrit
---
...
```{r}
write_rmd_body <- function(line_of_meta_data, has_text, has_photos, text_only_version = FALSE){
...
write_rmd <- function(line_of_meta_data){
  ...
  if (line_of_meta_data$has_photos == "yes" & line_of_meta_data$has_text == "yes") {
    ...
    rmd_to_write <- write_rmd_body(line_of_meta_data, has_text = TRUE, has_photos = TRUE)
    
    # write text-only version
    yaml_to_write_text_version <- write_yaml_header(line_of_meta_data, is_text_only_version = TRUE)
    write_lines(yaml_to_write_text_version, str_c(here("content", "manuscripts",
                                      str_c(line_of_meta_data$Catalogue_id, "-text-only.Rmd"))))
    rmd_to_write_text_version <- write_rmd_body(line_of_meta_data, has_text = TRUE, has_photos = FALSE, text_only_version = TRUE)
    ...
    write_lines(yaml_to_write, str_c(here("content", "manuscripts",
                                      str_c(line_of_meta_data$Catalogue_id, ".Rmd"))))  
    rmd_to_write <- write_rmd_body(line_of_meta_data, has_text = FALSE, has_photos = TRUE)
    ...
    write_lines(yaml_to_write, str_c(here("content", "manuscripts",
                                      str_c(line_of_meta_data$Catalogue_id, ".Rmd"))))  
    rmd_to_write <- write_rmd_body(line_of_meta_data, has_text = TRUE, has_photos = FALSE)
  }
