...
photoSizeHigh: {photo_size_high}{images_total_line}{image_current_line}
categories:
  - sanskrit
---
...
', .open = "{{", .close = "}}")
  ...
  initiate_zoom_effect <- case_when(
    has_text & has_photos ~ 'initiate_zoom_effect(photo_info)',
    has_photos ~ "",
...
write_rmd <- function(line_of_meta_data, current_image_num = FALSE){
  ...
  if (line_of_meta_data$has_photos == "yes" & line_of_meta_data$has_text == "yes") {
    ...
    image_info <- aws_content %>% 
      filter(manuscript == line_of_meta_data$Catalogue_id, filepath != paste0(line_of_meta_data$Catalogue_id, "/"))
